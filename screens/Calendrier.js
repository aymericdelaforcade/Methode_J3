...
/* Faire décalage des J possible par action
  Fait que supprimer une matière ne supprime pas les J associés
   ...
   Faire le décalage dès le change de stockage_des_J
    FONCTION PREMIUM: pouvoir prévoir un J
    ...
    faire le fait de pouvoir enlever un J décalé fait + ne pas faire un J décalé

    console.log(JSON.stringify(stockage_des_J_externe, null, 2)) permet de tout voir sur le console.log()
*/

/* PROBLEMES:
Absolument vérifier que si un J entre dans pas faire ou fait il faut absolument l'enlever des J reportés, car rien ne le fait

GROS PROBLEME: tout est basé sur le fait que pour qu'un J soit détécté comme décalé son index est de -1, mais si il arrive en même temps qu'un jour normal, son index sera pas de -1

J qui revient après être supprimé et avoir rajouté un J 

Pas de suppression et modif des matière

Problème modif et suppression matière
*/
...
export default function Calendrier({navigation}) {  
  ...
  function diminuer_jour () {
    const diminution_jour_1_pour_calculs = moment(jour_1_pour_calculs).subtract(2, 'days')
    ...
    const diminution_jour_2_pour_calculs = moment(jour_2_pour_calculs).subtract(2, 'days')
  ...
  function augmenter_jour () {
    const augmentation_jour_1_pour_calculs = moment(jour_1_pour_calculs).add(2, 'days')
    ...
    const augmentation_jour_2_pour_calculs = moment(jour_2_pour_calculs).add(2, 'days')
  ...
  function changerfenetre ({navigation}){
  ...
  const renderItem = ({item}) => { //Ce que rend la flat list qui gère la liste des matière
    ...
    if (stockage_des_J[item.id2].J_fait.some(element => element.numberduJ == number_du_J && element.numeroJ == item.numeroJ)) { //fonctionne aussi pour les J reportés fait, considérés comme number du J = -1
      opacité = 0.3
    }
    ...
    return(
      <TouchableOpacity style={[
        ...
        delayLongPress={200}  
        onLongPress={() => {traitement_du_J_fait(item.id2, number_du_J, item.numeroJ, item.decalage)}} //PROBLEME ICI -- résolu
        onPress={() => {
  ...
  const renderItem2 = ({item}) => { //Ce que rend la flat list qui gère le rendu des J de la box 2, première ligne permet de retourner le numéro du J, la deuxième la matière
    ...
    if (numéro_du_J == undefined && item.decalage == ''){ //gère le cas où c'est un J rajouté simple
      numéro_du_J = moment(jour_2, 'DD-MM-YYYY').diff(moment(stockage_des_J[item.id2].jour_séléctionné[0], 'DD-MM-YYYY'), 'days')
      if (CouleuraAfficher === true){ //affiche la couleur de la matière
        const matièreetcouleur = ListeMatieres.find(item => item[0] === matièreactuelle)
        if (matièreetcouleur !== undefined && matièreetcouleur !== null) {
          couleur_du_J2 = matièreetcouleur[1]
    ...
    <GestureDetector gesture={pinchGesture}>
        <View style={[styles.principal, {backgroundColor: CouleurFond}]}>
          ...
          <View style={styles.style_du_changer_jour_view}>    
            <TouchableOpacity style={[styles.style_du_changer_jour_touchable1, {backgroundColor: DarkMode ? CouleurActive: CouleurBouton}]}><Text style={styles.style_des_text_du_choix_nb}>2J</Text></TouchableOpacity>
